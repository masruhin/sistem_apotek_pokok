...
(function (factory) {
	...
	var Style = function () {
		...
		this.current = function (rng, elTarget) {
			...
			oStyle["font-bold"] = document.queryCommandState("bold")
				? "bold"
				: "normal";
			oStyle["font-italic"] = document.queryCommandState("italic")
				? "italic"
				: "normal";
			oStyle["font-underline"] = document.queryCommandState("underline")
				? "underline"
				: "normal";
			oStyle["font-strikethrough"] = document.queryCommandState("strikeThrough")
				? "strikethrough"
				: "normal";
			...
			if (!rng.isOnList()) {
				...
				var bUnordered =
					$.inArray(oStyle["list-style-type"], aOrderedType) > -1;
			...
			if (elPara && elPara.style["line-height"]) {
				...
				var lineHeight =
					parseInt(oStyle["line-height"], 10) /
					parseInt(oStyle["font-size"], 10);
			...
			var nativeRange = function () {
				if (bW3CRangeSupport) {
					...
					textRange.setEndPoint(
						"EndToEnd",
						bp2textRange({ cont: ec, offset: eo })
					);
					return textRange;
				}
			};
	...
	var Toolbar = function () {
		...
		this.update = function ($toolbar, oStyle) {
			...
			btnState('button[data-event="justifyLeft"]', function () {
				return (
					oStyle["text-align"] === "left" || oStyle["text-align"] === "start"
				);
			});
		...
		var hToolbarAndPopoverClick = function (event) {
			...
			if ($btn.length > 0) {
				...
				if ($.inArray(sEvent, ["backColor", "foreColor"]) !== -1) {
					...
					dialog
						.showLinkDialog($editable, $dialog, linkInfo)
						.then(function (sLinkUrl, bNewWindow) {
							editor.restoreRange($editable);
					...
					editor.saveRange($editable);
					dialog
						.showVideoDialog($editable, $dialog, videoInfo)
						.then(function (sUrl) {
							editor.restoreRange($editable);
			...
			$dropzone
				.on("drop", function (event) {
					...
					if (dataTransfer && dataTransfer.files) {
						var oLayoutInfo = makeLayoutInfo(
	...
	var Renderer = function () {
		...
		var tplToolbarInfo = {
			...
			paragraph: function (lang) {
				...
				var centerButton = tplIconButton(
					"fa fa-align-center icon-align-center",
					{
		...
		var tplExtraShortcuts = function (lang, options) {
			...
			for (var key in extraKeys) {
				if (extraKeys.hasOwnProperty(key)) {
					body +=
						"<tr><td>" + key + "</td><td>" + extraKeys[key] + "</td></tr>";
				}
			}
		...
		this.createLayout = function ($holder, options) {
			...
			if (options.height > 0) {
				$('<div class="note-statusbar">' + tplStatusbar() + "</div>").prependTo(
					$editor
				);
			}
