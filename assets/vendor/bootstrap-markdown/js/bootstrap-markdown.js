...
!(function ($) {
				...
				for (y = 0; y < btnGroups.length; y++) {
					...
					for (z = 0; z < buttons.length; z++) {
						var button = buttons[z],
							...
							buttonIcon =
								button.icon instanceof Object
									? button.icon[this.$options.iconlibrary]
									: button.icon,
	...
	var analyzeMarkdown = function (e) {
		...
		if (
			(e.type == "focusin" || e.type == "click") &&
			$docEditor.length == 1 &&
			typeof $docEditor[0] == "object"
		) {
			el = $docEditor[0].activeElement;
			if (!$(el).data("markdown")) {
				if (
					typeof $(el).parent().parent().parent().attr("class") ==
						"undefined" ||
					$(el).parent().parent().parent().attr("class").indexOf("md-editor") <
						0
				) {
					if (
						typeof $(el).parent().parent().attr("class") == "undefined" ||
